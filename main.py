...
async def start(update, context):
    ...
    keyboard = [authors[i:i+2] for i in range(0, len(authors), 2)] + [['Резюме']]
...
async def author_received(update, context):
    ...
    authors = get_unique_authors('Данные')
    if update.message.text not in authors and update.message.text!='↩️Возврат к выбору года' and  update.message.text != 'Резюме':
...
async def vars_received(update, context):
    ...
    if context.user_data['summary'] == 'summary':
        all_messages = ['Последние прогнозы по ключевым переменным. В формате Банк России / Аналитики / МЭР \n\n']
...
async def pred_received(update, context):
    ...
    if context.user_data['summary'] == 'summary':
        ...
        keyboard = [['Заново'], ['Выбрать другой документ'], ['Завершить']]
