    ...
    for year in data_all.index:
        ...
        for x in ['Банк России', 'Аналитики', 'МЭР']:
            ...
            if x != 'МЭР':
                st = st + ' / '
        t = f'{st}'.replace('.', ',')
        r = real[real.iloc[:, 0] == name_v][int(year)].values[0]
        if pd.notna(r):
            r = round(float(r), 1)
            r = str(r).replace('.', ',')
            t = t + f" (факт: {r})"
...
async def year_received(update, context):
    ...
    elif update.message.text == 'Последний прогноз':
...
async def vars_received(update, context):
    ...
    if context.user_data['summary'] == 'summary':
        max_year_cb = str(max(map(int, get_unique_doc_years('Банк России'))))
        imax = 0
        docmax =''
        doc_item_max = ''
        directory_year = f"Данные/Банк России/{max_year_cb}"
        for item in os.listdir(directory_year):
            full_path = os.path.join(directory_year, item)
            if os.path.isdir(full_path):
                if 'Базовый прогноз' in item.partition('-')[0]:
                    if int(item.partition('-')[2].partition('-')[0]) > imax:
                        ...
                        docmax = item.partition('-')[0] + '-' + item.partition('-')[2].partition('-')[2]
                        doc_item_max = item
            directory_cb = f"Данные/Банк России/{max_year_cb}/{doc_item_max}"

        max_year_a = str(max(map(int, get_unique_doc_years('Аналитики'))))
        directory_year = f"Данные/Аналитики/{max_year_a}"
        months = []
        for item in os.listdir(directory_year):
            if item in month_order:
                months = months + [item]
        docmax_a = max(months, key=lambda x: month_order.index(x))
        directory_a = f"Данные/Аналитики/{max_year_a}/{docmax_a}"

        max_year_me = str(max(map(int, get_unique_doc_years('МЭР'))))
        directory_year = f"Данные/МЭР/{max_year_me}"
        months = []
        for item in os.listdir(directory_year):
            if item[:3] in month_order:
                months = months + [item]
        docmax_me = max(months, key=lambda x: month_order.index(x[:3]))
        directory_m = f"Данные/МЭР/{max_year_me}/{docmax_me}"

        all_messages = [f'Последние прогнозы по ключевым переменным. В формате \nБанк России ({doc_item_max.split('-')[-1]}-{max_year_cb}) / Аналитики ({docmax_a}-{max_year_a}) / МЭР ({docmax_me[:3]}-{max_year_me}) \n\n']
